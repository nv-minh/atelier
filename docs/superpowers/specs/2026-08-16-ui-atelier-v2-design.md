...
**Ảnh chụp của `/`, `/browse`, `/stats` không byte-identical giữa hai lần chạy, kể cả cùng một commit.** Ba route này fetch ảnh/dữ liệu qua mạng (`getDemoWords()` ở landing, ảnh Pexels ở `/browse`, câu trích dẫn hằng ngày ở `/stats`); đo trực tiếp: hai lần chụp liên tiếp trên cùng commit lệch nhau ở đúng các route đó, max diff 46/255, < 1% pixel, luôn khoanh đúng vùng ảnh/dữ liệu động. So ảnh những route này bằng **bbox + % pixel lệch** (< 1%, max diff < 60/255, bbox nằm trong vùng ảnh/dữ liệu — không phải nav/layout), không kỳ vọng byte-identical.
...
**R26 — Ngân sách font 120 KB (§11) đã vỡ TRƯỚC KHI Plan 1 Task 5 bắt đầu, không phải do task này.** Đo thật trên bản production: tổng byte font sau khi đổi sang Be Vietnam Pro là 286,9 KB (đã bỏ weight 800 không dùng), không phải < 120 KB như spec dự đoán ("ba họ → hai họ, tổng byte giảm"). Trích `@font-face` thật từ CSS build xác định file lớn nhất (155 KB, hơn nửa tổng) là `__Noto_Sans_Mono` subset `latin-ext` — đúng subset mang glyph IPA, một file variable `font-weight:100 900` **có từ `ec6206e` (một ngày trước nhánh migration)**, Task 5 không đụng tới. Be Vietnam Pro tự thân (4 weight × 2 subset = 8 file) chỉ ~100–110 KB, dưới ngân sách nếu đứng riêng.

**Việc giao lại cho Plan 8 (cổng đo cuối):** ngân sách 120 KB xung đột thật với quyết định giữ IPA đúng đã chốt từ `ec6206e` — hai mục tiêu không cùng đạt được với font mono hiện tại. Plan 8 phải chọn một trong hai: (a) xét lại con số 120 KB trong §11 cho đúng thực tế đánh đổi, hoặc (b) tìm font mono/IPA nhỏ hơn 155 KB mà vẫn qua được `npm run check:type`. **Không tự ý đổi font mono ở bất kỳ task nào trước Plan 8** — đó là quyết định cần đo kỹ, không phải sửa vội.

**R25 — Đổi tên 1:1 một token cũ sang token mới không phải lúc nào cũng trung tính.** Ở Plan 1 Task 4: `.border-line` cũ hard-code alpha 10% trong định nghĩa CSS; `border-hairline` mới là màu Tailwind Tier 1 thật, dạng trần không `/N` trả `<alpha-value>` = 1 (đặc), không phải 0.1. Sed ngây thơ biến mọi viền mờ thành viền đặc — đã sửa bằng cách viết đủ `/10` ở cả 92 chỗ. Ngược lại, `text-soft` cũ là class thuần không phải màu Tailwind nên `text-soft/70` xưa nay **âm thầm biên dịch ra không gì cả** (đúng bẫy đã biết, xem memory `vocab-master-tailwind-tokens`); đổi sang `text-fg-muted/70` (màu thật) làm modifier **lần đầu tiên chạy đúng** — chấp nhận, đây là sửa đúng ý định gốc chứ không phải hồi quy (đo bbox+%pixel xác nhận). **Bài học cho mọi task đổi tên class còn lại:** kiểm xem class cũ có phải màu Tailwind thật hay chỉ là CSS thuần — hai trường hợp phản ứng với `/N` khác hẳn nhau.
...
| Đưa tổng byte font về dưới 120 KB (§11, §7 R26) | Vỡ từ `ec6206e` (trước cả migration này) vì file IPA của Noto Sans Mono nặng 155 KB; giao Plan 8 chọn xét lại ngân sách hoặc đổi font mono |
